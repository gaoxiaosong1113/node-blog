...
!function ($) {
    ...
    var commands = {
        ...
        image: function (delegate) {
            ...
            var s = '<div class="modal fade" data-backdrop="static" role="dialog" aria-hidden="true">' +
            ...
            // $modal.find('button.btn-default').click(cancel);
            $modal.find('.btn-success').click(function () {
                // clear error:
                ...
                try {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (res[0] == 'success') {
                                delegate.unselect();
                                $alert.removeClass('hidden').addClass('alert-danger show').text('文件上传成功！文件名为：' + res[1].filename);
                                $modal.modal('hide');
                                delegate.unselect();
                                var s = '\n![' + text.replace('[', '').replace(']', '') + '](' + res[1].fileUrl + ')\n';
                                delegate.paste(s);
                                delegate.setSelection(delegate.getCaretPosition() + 1, delegate.getCaretPosition() + s.length - 1);
                                $modal.modal('hide');
                            }
                            else {
                                $alert.removeClass('hidden').addClass('alert-danger show').text('文件上传失败！原因是：' + res[1].msg);
                            }
                        },
                        ...
                        error: function () {
                            $alert.removeClass('hidden').addClass('alert-danger show').text('连接不到服务器，请检查网络！');
                        }
                    });
                    //
                    // var text = $text.val();
    ...
    $.fn.markdown.defaults = {
        ...
        upload_image_url: 'http://www.gxspp.com/upload',
        upload_file_url: ''
    };
